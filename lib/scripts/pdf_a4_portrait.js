...
function renderNow() {
  ...
  system.stdout.write(JSON.stringify({ filename: filename, numPages: numPagesOutput }))
  exit(null)
}
...
function createSection(section, content, options) {
  ...
  return {
    ...
    contents: phantom.callback(function(pageNum, numPages) {
      ...
      numPagesOutput = numPagesFinal

      if (pageNumFinal === 1 && !html) html = o.first || c.first
      if (pageNumFinal === numPages && !html) html = o.last || c.last
      return (html || o.default || c.default || '').replace(/{{page}}/g, pageNumFinal).replace(/{{pages}}/g, numPagesFinal) + content.styles
    })
  }
}
